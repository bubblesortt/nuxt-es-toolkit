...
`src/module.ts` defines the Nuxt module and its public options. `src/utils/imports.ts` plans and validates auto-imports independently of Nuxt. `src/runtime/` contains root, compat, FP, Map, and Set export barrels; update them deliberately when changing import resolution. Shared helpers live in `src/utils/`.
